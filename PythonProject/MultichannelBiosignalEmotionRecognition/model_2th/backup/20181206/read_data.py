...
def _get_samples_data(people_list, trial_list, path, windows=9, overlapping=8, classify_object_name=0):
    ...
    for people in people_list:
        for trial in trial_list:
            ...
            if label_value[classify_object_name] >= 5.7579:
                label = 0
            elif label_value[classify_object_name] < 5.7579:
                label = 1
...
def _save_samples(datas_result, labels, people_list, train_flag):
    ...
    if len(people_list) == 1:
        ...
        if not os.path.isdir(os.path.join("../common/samples_single_people", "s"+str(people_list[0]))):
            os.makedirs(os.path.join("../common/samples_single_people", "s"+str(people_list[0])))
        if train_flag:
            np.save("../common/samples_single_people/s"+str(people_list[0])+"/train_datas", datas_result)
            np.save("../common/samples_single_people/s"+str(people_list[0])+"/train_labels", labels)
        else:
            np.save("../common/samples_single_people/s"+str(people_list[0])+"/test_datas", datas_result)
            np.save("../common/samples_single_people/s"+str(people_list[0])+"/test_labels", labels)
    if len(people_list) > 1:
        if train_flag:
            np.save("../common/samples_all_people"+"/train_datas", datas_result)
            np.save("../common/samples_all_people"+"/train_labels", labels)
        else:
            np.save("../common/samples_all_people"+"/test_datas", datas_result)
            np.save("../common/samples_all_people"+"/test_labels", labels)
