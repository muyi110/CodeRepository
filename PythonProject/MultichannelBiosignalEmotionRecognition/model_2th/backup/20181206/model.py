...
if __name__ == "__main__":
    ...
    train_index = [5]
    test_index = [5]
    train_trial_list = list(permutation[0:8]) + list(permutation[16:40])
    test_trial_list = list(permutation[8:16])
    # 获取生理信号数据
    ...
    datas_train = np.load("../common/samples_single_people/s7/train_datas.npy") 
    train_labels = np.load("../common/samples_single_people/s7/train_labels.npy") 
    datas_test = np.load("../common/samples_single_people/s7/test_datas.npy")  
    test_labels = np.load("../common/samples_single_people/s7/test_labels.npy")  
    ...
    tcn.fit(X=datas_train, y=train_labels, n_epochs=121, X_test=datas_test, y_test=test_labels)
    tcn.restore()
    total_acc_test = 0
    ...
    for i in range(len(test_labels) // 8):
        ...
        loss_test, acc_test = tcn._session.run([tcn._loss, tcn._accuracy], 
                                        feed_dict={tcn._X:X_batch_test, tcn._y:y_batch_test, tcn._training:False})
        total_acc_test += acc_test
        total_loss_test += loss_test
    print("Test accuracy: {:.4f}%\t  Test loss: {:.6f}".format((total_acc_test / (len(test_labels) // 8))*100, total_loss_test/(len(test_labels) // 8)))
