...
def get_samples_data(path, people_list, windows=4, overlapping=3):
    '''
      windows: 划分的时间窗口长度
...
def read_data(path=SAMPLES_PATH, people_list = list(range(0,32)), windows=4, overlapping=3, raw_data=False):
    # datas 和 labels 都是 list. datas 中的每一项是 shape=(32, 128*windows) 的数组
    datas, labels = get_samples_data(path, people_list, windows, overlapping)
    datas_result = []
    for data in datas:
        data_list = []
        if not raw_data:
