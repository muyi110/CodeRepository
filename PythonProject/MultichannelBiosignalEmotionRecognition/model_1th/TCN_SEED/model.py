...
if __name__ == "__main__":
    # 获取生理信号数据
    datas, labels = read_data(windows=9, overlapping=8)
    ...
    # 开始将数据集划分为训练集和测试集
    np.random.seed(42)
    datas_train = datas[:1938]
    train_labels = labels[:1938]
    datas_test = datas[-1336:]
    test_labels = labels[-1336:]
    del datas # 释放内存
    ...
    num_channels = [100] # 有多少层，及每一层包含的神经元个数（这里的一层指一个 block）
    kernel_size = 5   # 卷积核大小  
    batch_size = 16
